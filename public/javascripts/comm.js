...
var bgColor = ['bg-color-blueDark', 'bg-color-greenLight', 'bg-color-greenDark', 'bg-color-red', 'bg-color-orange', 'bg-color-orangeDark', 'bg-color-pink', 'bg-color-pinkDark', 'bg-color-purple', 'bg-color-grayDark', 'bg-color-magenta', 'bg-color-emerald', 'bg-color-teal', 'bg-color-indigo', 'bg-color-violet', 'bg-color-magenta', 'bg-color-crimson', 'bg-color-brown'];
...
String.prototype.isChs = function () {
    var rg = new RegExp('^([\\u4e00-\\u9fa5]+|[a-zA-Z0-9]+)$', 'g');
    return rg.test(this);
};
...
Array.prototype.Max = function (p) {
    var r = 0;
    this.Foreach(function (o) { r = o[p] > r ? o[p] : r; });
    return r;
};
...
Array.prototype.Sum = function (p) {
    var r = 0;
    for (var i = 0; i < this.length; i++) { if (typeof p == 'string') { if (typeof this[i][p] == 'number') { r += this[i][p]; } else { r += parseFloat(this[i][p]) || 0; } } else { r += p(this[i], i); } }
    return r;
};
Array.prototype.Count = function (p) {
    if (p) {
        var r = 0;
        for (var i = 0; i < this.length; i++) { r += p(this[i]); }
        return r;
    }
    return this.length;
};
Object.Clone = function (o) {
    ...
    if (o instanceof Array) {
        ...
        r = new Object();
        for (var i in o) r[i] = Object.Clone(o[i]);
        return r;
    }
    ;
}
...
function getDayExp() {
    ...
    switch (type) {
        case 1:
            return ('^[1-9]{1}$|^[12]\\d$|^3[01]$');
        case 2:
            return ('^[1-9]{1}$|^[12]\\d$|30$');
        case 3:
            return ('^[1-9]{1}$|^1\\d$|^2[0-9]$');
        case 4:
            return ('^[1-9]{1}$|^1\\d$|^2[0-8]$');
    }
};
(function ($) {
    $.toJSON = function (o) {
