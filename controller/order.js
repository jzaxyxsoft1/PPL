...
exports.postsave = function (req, res) {
    ...
    var st = order.Status;
    OrderSvc.CreateOrder(order, req.currentUser, function (e, d) {
        if (st == '未付款') {
            OrderSvc.SubmitOrder(order._id, pUser, function (ee) {
                res.json({msg: ee == null, error: ee, ID: order._id, BillNum: order.BillNum});
            });
        }
        else {res.json({msg: e == null, error: e, ID: order._id, BillNum: order.BillNum})}
    })
}
