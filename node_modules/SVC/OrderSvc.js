...
var OrderSvc = exports.OrderSvc = {
    ...
    SubmitOrder: function (orderID, operator, cb) {
        Svc.db.Order.update({_id: orderID}, {$set: {Status: '未付款'}}, function(e){
            cb(e);
        })
    },
    /**
                    ...
                    async.parallel(
                        {
                            ...
                            trb: function (pcb) {Svc.db.TransferBill.findOne({_id: transferBillId}, pcb);}
                        },
                        function (e, result) {
                            org = result.org;
                            trBill = result.trb;
                    ...
                    async.parallel(
                        [
                            ...
                            function (pcb) {
                                ...
                                Svc.db.TransferBill.update({_id: trBill._id}, {$set: {Status: '已完成', Items: trBill.Items }}, function (e) { pcb(null);});
                            },
                            //库存更新
                            function (pcb) {
                            ...
                            function (pcb) {
                                _.each(trBill.Items, function (i) {
                                    ...
                                    oi.CompleteOperation.push({TransferBillNum: trBill.BillNum, Operator: operator, Time: Date.ToDateTimeString()});
