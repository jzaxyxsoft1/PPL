...
Array.sum = function (ary, f) {
    var _r = 0;
    if (typeof  f == 'string') {
        _.each(ary, function (i) { _r += i[f]});
    }
    else {
        _.each(ary, function (i) { _r += f(i)});
    }
    return _r;
};
Array.prototype.sum = function (f) {
    var _r = 0;
    if (typeof f == 'string') { this.forEach(function (i) { _r += i[f]}); }
    else {this.forEach(function (i) {_r += f(i)})}
    return _r;
};
Object.Clone = function () {
    if (typeof this == 'string' || typeof this == 'number' || typeof this == 'boolean' || this == undefined || this == null || this == NaN || this instanceof Function) {
        return  this;
    }
...
var Svc = {
     ...
     */
    IOrgObj: {"Stock": 1, "RnP": 1, "Storage": 1, 'User': 1, 'StockIn': 1, 'StockOut': 1, 'TransferBill': 1, 'Order': 1, 'Package': 1, 'SaleBill': 1, 'Communication': 1},
    /**
     * 实现简码的对象
     ...
     */
    ICreator: {'StockIn': 1, 'StockOut': 1, 'TranseferBill': 1, 'Order': 1, 'SaleBill': 1, 'Communication': 1},
    /**
     * 实现创建时间接口的对象
     * interface ICreateTime:{CreateTime:{Item1:日期时间字符串,Item2:年(Number),Item3:月(Number),Item4:日(Number)}}
     */
    ICreateTime: {'StockIn': 1, 'StockOut': 1, 'TranseferBill': 1, 'Order': 1, 'SaleBill': 1, 'Communication': 1},
    /**
     * 部门对象接口的对象
     ...
     */
    IBill: {'Order': 1, 'StockIn': 1, 'StockOut': 1, 'SaleBill': 1, 'TranseferBill': 1},
    /**
     * 更新审核对象
    ...
    beforInsert: function (type, obj, creator, callBack) {
        ...
        if ((Svc.ICreator[type] || Svc.UpdateAuditObj[type] || Svc.DeleteAuditObj[type] || Svc.IOrgObj[type] ) && _creator == null) {
            ...
            async.parallel(
                {
                    ...
                    isimcode: function (cb) {
                        if (Svc.ISimcode[type] && obj.Name) {
                            obj.Simcode = pinyin(obj.Name, '').toUpperCase();
                        }
                        cb(null);
                    },
                    ...
                    ibill: function (cb) {
                        if (Svc.IBill[type] || Svc.BillNumPre[type]) {
            ...
            async.parallel(
                {
                    ...
                    iorg: function (cb) {
                        if (Svc.IOrgObj[type]) {
                            _.each(obj, function (oi) { oi.Org = creator.Org});
                        }
                        else cb();
                    }
                }, callback);
    ...
    beforUpdate: function (type, query, option, creator, callBack) {
        ...
        option = args.shift();
        var _creator = args.length ? args.shift() : null;
        if ((Svc.ICreator[type] || Svc.UpdateAuditObj[type]) && _creator == null) {
            callback(Svc.ErrorType._未传递创建者实例);
            return;
        }
        var op = option['$set'] || option;
        if (op.Name && Svc.ISimcode[type]) { op.Simcode = pinyin(op.Name, '').toUpperCase(); }
        ...
        async.parallel({
            ...
            async.parallel({
                               ...
                               icreatetime: function (rcb) {
                                   if (Svc.ICreateTime[type] || Svc.ICreator[type] || Svc.UpdateAuditObj[type] || Svc.DeleteAuditObj[type]) {
                                       op.CreateTime = Date.ToCreateTime();
                                   }
                                   if (Svc.ICreator[type] || Svc.UpdateAuditObj[type] || Svc.DeleteAuditObj[type]) {
                                       op.Creator = _creator;
                                   }
    ...
    getBillNum: function (type) {
        if (Svc.BillNumPre[type]) {
            var d = Svc.GV.BillNum[type];
            d.Num = d.Num + 1;
            var t = new Date;
            t = t.getFullYear().toString() + (t.getMonth() + 1) + t.getDate();
            if (d.Date != t) {
                d.Date = t;
                d.Num = 1;
            }
     ...
     * 单号前缀
     */
    BillNumPre: {'Order': 'ORD', 'Package': 'PKG', 'StockIn': 'ISTK', 'StockOut': 'OSTK', 'TranseferBill': 'TRB', 'SaleBill': 'SL'},
    /**
     * 创建应收应付
    ...
    createRnP: function (owner, relativeOrg, summary, debit, credit, creator, remark, cb) {
        db.RnP.find({'Org.Value': owner.Value, 'RelativeOrg.Value': relativeOrg.Value}).sort({'CreateTime.Item1': -1}).limit(1).toArray(function (e, d) {
            var rm = d.length == 0 ? 0 : d[0].Remain;
            db.RnP.insert({
                              _id: db.RnP.ObjectID().toString(),
                              Creator: creator,
    ...
    /**
     * 检查库存单据状态
     * @param {Object} bill 库存单据实例
     */
    CheckStorageBillStatus: function (bill) {
        _.each(bill.Items, function (i) {
            i.RemainAmount = i.Amount - i.CompleteAmount;
            i.Cost = i.UnitCost * i.Amount;
            i.Status = i.RemainAmount == i.Amount ? '待执行' : i.RemainAmount > 0 ? '执行中' : '已完成';
            if (_.every(bill.Items, function (i) {return i.Status == '已完成'})) {
                bill.Status = '已完成'
            }
            else if (_.any(bill.Items, function (i) {return i.Status == '执行中'})) {
                bill.Status = '执行中'
            }
        });
        bill.Cost = bill.Items.sum('Cost');
    },
    SolutionTypes:[],
    Solutions:[],
    ErrorType: {
